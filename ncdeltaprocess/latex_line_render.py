...
# (Same-status runs and empty markers are folded earlier, at the op level, by
# ``coalesce_adjacent_string_ops`` — that is where attribute equality and
# string-vs-embed can be checked safely. This render-level fold only pairs the
# two *different*-status runs of a replacement, which is not an op merge.)
...
def pair_adjacent_diff_replacements(latex: str) -> str:
    ...
    ``\\replaced{new}{old}\\hspace{0pt}``. The diff renders a word-level edit as
    a *replacement* — an added run (new text) immediately followed by a deleted
    run (struck-through old text) — i.e. two ``changes``-package commands.
    ``\\replaced{new}{old}`` is the package's own single-command form for exactly
    this, rendering identically (new text, then old struck) while roughly halving
    the comparatively slow ``changes`` commands in heavily-revised text.

    Conservative: only **brace-free** operands are folded, so a pair wrapping
    nested formatting (e.g. deleting italic and inserting bold —
    ``\\added{\\textbf{new}}\\hspace{0pt}\\deleted{\\emph{old}}``) is left as the
    explicit add/delete. Only insert-first (added-then-deleted) pairs match, so
    delete-before-insert ordering is never folded (``\\replaced`` renders
    new-first and would otherwise flip it).
    """
    if not latex:
        return latex
